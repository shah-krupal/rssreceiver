...
public class WebHookServiceImpl implements WebHookService {
    ...
    public String hook(Channel channel) {
        ...
        email.setSubject(channel.getTitle());
        email.setLink(channel.getUri());
        email.setBody(channel.getDescription() + LocalTime.now());
        try {
            System.out.println("channel "+channel.getTitle() );
//            Query query = new Query() ;
//            query.addCriteria(Criteria.where("feedTitle").is)
            FeedSubscriber feedData = feedSubscriberRepo.findByFeedTitle(channel.getTitle());
            System.out.println(feedData.getEmailIds());
            System.out.println(feedData);
            List<String> emailList = feedData.getEmailIds();
            if(emailList != null)
            {
                System.out.println(emailList);
                return mailing2(email, emailList) ;
            }
            else
            {
                return "DONE" ;
